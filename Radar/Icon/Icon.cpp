...
enum status_e icon::showDetails(icon const &icon)
{
    ...
    for (int i = 0; i < 16; i++) // Iterate through the pixels
    {
        pixel::showDetail(icon.The_Pixels[i]); // Show the details of each pixel
    }
    return STATUS_OK; // Return status ok
};

/// @details This function prints the details of the icon to the console. This includes the id of the icon and the brightness of each pixel. This function is used for debugging.
/// @note This function is a wrapper for the showDetails function. It is used to make the code more portable.
enum status_e icon::showDetails()
{
    return icon::showDetails(*this); // Show the details of the icon object that called the function
};

/// @details This function prints the details of the icon to the console. This includes the id of the icon and the brightness of each pixel. This function is used for debugging.
/// @note This function is a wrapper for the showDetails function.
...
/// @note use #showDetails(icon const &icon) instead.
enum status_e icon::showIconDetail(icon const &icon)
{
    return icon::showDetails(icon); // Return status ok
};

/// @details This function prints the details of the icon to the console. This includes the id of the icon and the brightness of each pixel. This function is used for debugging.
/// @note This function is a wrapper for the showDetails function.
/// @note This function only exists because it is required by the specification however has a terrible name and should not be used.
/// @note use showDetails() instead.
/// @par Example:
/// @include main5.cpp
...
enum status_e icon::setPixel(uint8_t pixelId, float x, float y, uint8_t Brightness)
{
    ...
    The_Pixels[pixelId].setX(x);                   // Set the x coordinate of the pixel
    The_Pixels[pixelId].setY(y);                   // Set the y coordinate of the pixel
    The_Pixels[pixelId].setBrightness(Brightness); // Set the brightness of the pixel
    return STATUS_OK;                              // Return status ok
};
...
enum status_e icon::initialiseAsDefaultDiagonalLine(uint8_t Brightness)
{
    ...
    {
        The_Pixels[pixelId].setX(pixelId);             // Set the x coordinate of the pixel
        The_Pixels[pixelId].setY(pixelId);             // Set the y coordinate of the pixel
        The_Pixels[pixelId].setBrightness(Brightness); // Set the brightness of the pixel
    }
    return STATUS_OK; // Return status ok
};
